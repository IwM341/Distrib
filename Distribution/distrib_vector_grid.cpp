...
int main(int argc,char **argv){
    ...
    std::map<std::string,std::string> params =
        {{"lh_out","lh.bmat"},{"hl_out","hl.bmat"},
         ...
         {"type","double"},{"body",""},
         {"config_out","config.txt"}};
    for(const auto & item:params){
        if(cmd_params.find(item.first) == cmd_params.not_found()){
            cmd_params.put(item.first,item.second);
        }
    }
    ...
    size_t work_rogress = 0;
    using namespace std::string_literals;
    show_progress(0,"HL progress: ");
    ...
    for(size_t i=0;i<S_HL.Grid.size()-1;++i){
        ...
        for(size_t j = 0;j<S_HL.values[i].Grid.size()-1;++j){
            ...
            ++work_rogress;
            show_progress(work_rogress/workSize,"HL progress: ");
        }
    }
    print();
    show_progress(0,"LH progress: ");
    for(size_t i=0;i<S_LH.Grid.size()-1;++i){
        ...
        for(size_t j = 0;j<S_LH.values[i].Grid.size()-1;++j){
            ...
            TrajectoryIntegral1(G,e_nd,l_nd,TI,
                               ...
                               mk,mp,-delta_mk,dF_H<ELASTIC,PROTON>(),PhiFactorSS,100000);
            ++work_rogress;
            show_progress(work_rogress/workSize,"LH progress: ");
        }
    }
    print();
